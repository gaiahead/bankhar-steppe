...
class Element{constructor(id){this.id=id;this.style={};this.hidden=['help','settings'].includes(id);this.listeners={};this.dataset={};this.value='';this.classList={add(){},toggle(){}};this.captured=new Set();}addEventListener(k,fn){(this.listeners[k]??=[]).push(fn);}setAttribute(){}focus(){}closest(){return null;}setPointerCapture(id){this.captured.add(id);}hasPointerCapture(id){return this.captured.has(id);}releasePointerCapture(id){this.captured.delete(id);}getBoundingClientRect(){return{x:0,y:0,left:0,top:0,width:116,height:116};}getContext(){return gl;}dispatch(type,e){for(const f of this.listeners[type]||[])f({target:this,preventDefault(){},...e});}}
...
const doc={getElementById:get,querySelectorAll:()=>touch,body:get('body'),addEventListener(){},hidden:false};
...
key('KeyQ');step(90);assert(ctx.__bankharDebug.dog.speed>14);
